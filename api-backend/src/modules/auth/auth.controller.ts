...
export const registerClient = async (
  req: Request<{}, {}, IRegisterClientDTO>,
  res: Response
) => {
  const {
    name,
...
export const registerProvider = async (
  req: Request<{}, {}, IRegisterProviderDTO>,
  res: Response
) => {
  console.log("PAYLOAD RECEBIDO DO FRONT:", req.body);
  const {
    razaoSocial,
    nomeFantasia,
    cnpj,
...
export const login = async (req: Request<{}, {}, ILoginDTO>, res: Response) => {
  ...
  try {
    ...
    user = await dbGet<IDbUser>("SELECT * FROM CLIENTES WHERE EMAIL = ?", [
      email,
    ]);
    if (user) {
      userName = user.NOME;
    ...
    if (!user) {
      user = await dbGet<IDbUser>("SELECT * FROM PRESTADORES WHERE EMAIL = ?", [
        email,
      ]);
      if (user) {
    ...
    if (!user) {
      user = await dbGet<IDbUser>("SELECT * FROM ADMINS WHERE EMAIL = ?", [
        email,
      ]);
...
export const loginAdmin = async (
  ...
  try {
    const admin = await dbGet<IDbUser>("SELECT * FROM ADMINS WHERE EMAIL = ?", [
      email,
    ]);
